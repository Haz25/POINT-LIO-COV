...
PointCovVector feats_down_body_cov;
PointCovVector feats_down_world_cov;
std::vector<PointPlane> ptpl_list;
std::vector<V3D> pbody_list;
std::vector<PointVector> Nearest_Points; 
std::vector<PointCovVector> Nearest_PointCovs; 
std::shared_ptr<IVoxType> ivox_ = nullptr;                    // localmap in ivox
std::shared_ptr<IVoxCovType> ivoxCov_ = nullptr;
...
V3D alpha(V3D::Zero()), jerk(V3D::Zero());
...
Eigen::Matrix<double, 24, 1> get_f_input(state_input &s, const input_ikfom &in, const double dt)
{
	...
	vect3 a_inertial = s.rot * (in.acc-s.ba); // .normalized()
	for(int i = 0; i < 3; i++ ){
		res(i) = s.vel[i] + 0.5 * (a_inertial[i] + s.gravity[i]) * dt;
		res(i + 3) = omega[i]; 
		res(i + 12) = a_inertial[i] + s.gravity[i]; 
	}
	return res;
...
Eigen::Matrix<double, 30, 1> get_f_output(state_output &s, const input_ikfom &in, const double dt)
{
	...
	vect3 a_inertial = s.rot * s.acc; // .normalized()
	for(int i = 0; i < 3; i++ ){
		res(i) = s.vel[i] + (a_inertial[i] + s.gravity[i]) * dt / 2 + jerk(i) * dt * dt / 6;  // pos
		res(i + 3) = s.omg[i] + alpha(i) * dt / 2;                                          // rot
		res(i + 12) = a_inertial[i] + s.gravity[i] + jerk(i) * dt / 2;                      // vel
		res(i + 15) = alpha(i);                                                             // omg
		res(i + 18) = jerk(i);                                                              // acc
	}
	return res;
}
...
Eigen::Matrix<double, 30, 30> df_dx_output(state_output &s, const input_ikfom &in)
{
	...
	cov.template block<3, 3>(0, 12) = Eigen::Matrix3d::Identity();                                       // (pos, vel)     F23
	cov.template block<3, 3>(12, 3) = -s.rot*MTK::hat(s.acc); // .normalized().toRotationMatrix()        // (vel, rot)     F31
	cov.template block<3, 3>(12, 18) = s.rot; //.normalized().toRotationMatrix();                        // (vel, acc)     F38
	// Eigen::Matrix<state_ikfom::scalar, 2, 1> vec = Eigen::Matrix<state_ikfom::scalar, 2, 1>::Zero();
	// Eigen::Matrix<state_ikfom::scalar, 3, 2> grav_matrix;
	// s.S2_Mx(grav_matrix, vec, 21);
	cov.template block<3, 3>(12, 21) = Eigen::Matrix3d::Identity(); // grav_matrix;                      // (vel, gravity) F36
	cov.template block<3, 3>(3, 15) = Eigen::Matrix3d::Identity();                                       // (rot, omg)     F17
	return cov;
}
...
void h_model_input(state_input &s, Eigen::Matrix3d cov_p, Eigen::Matrix3d cov_R, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	//cout << "[in] effect_num_k: " << effect_num_k << endl;
...
void h_model_output(state_output &s, Eigen::Matrix3d cov_p, Eigen::Matrix3d cov_R, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	for (int j = 0; j < time_seq[k]; j++)
	{
		...
		ptpl.cov_b = pointCov_body_j.cov;
		ptpl.cov_w = calcWorldCov(p_body, pointCov_body_j.cov, kf_output);

		{
			auto &points_near = Nearest_Points[idx+j+1];
			auto &pointCovs_near = Nearest_PointCovs[idx+j+1];
			
			double t_knn = omp_get_wtime();
            ivox_->GetClosestPoint(point_world_j, points_near, NUM_MATCH_POINTS); // 
			ivoxCov_->GetClosestPoint(pointCov_world_j, pointCovs_near, NUM_MATCH_POINTS);
			knn_time += omp_get_wtime() - t_knn; 
			...
			{
				point_selected_surf[idx+j+1] = false;
				//if (esti_plane(pabcdf, points_near, plane_thr)) //(planeValid)
				//if (esti_plane(pabcdf, pointCovs_near, plane_thr))
				if (esti_plane(ptpl, pointCovs_near, plane_thr))
				{
					//pabcd(0) = pabcdf(0);
					//pabcd(1) = pabcdf(1);
					//pabcd(2) = pabcdf(2);
					//pabcd(3) = pabcdf(3);
					pabcd(0) = ptpl.normal(0);
					pabcd(1) = ptpl.normal(1);
					pabcd(2) = ptpl.normal(2);
					pabcd(3) = ptpl.d;
					float pd2 = fabs(pabcd(0) * point_world_j.x + pabcd(1) * point_world_j.y + pabcd(2) * point_world_j.z + pabcd(3));
					// V3D norm_vec;
					// M3D Rpf, pf;
	...
	//cout << "[out] effect_num_k: " << effect_num_k << endl;
